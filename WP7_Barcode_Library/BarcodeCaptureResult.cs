...
namespace WP7_Barcode_Library
{
    ...
    public class BarcodeCaptureResult
    {
        ...
        {
            BarcodeFormat = WP7BarcodeManager.ScanMode; //Set barcode type for these results
        ...
        public BarcodeCaptureResult(BitmapImage bmpBarcodeImage) : this()
        {
        ...
        public BarcodeCaptureResult(System.IO.Stream ImageStream) : this()
        {
        ...
        public void SaveJPEGToIsolatedStorage(string strFilename)
        {
            try
            {
                ...
                WP7BarcodeManager.SaveJPEGToIsolatedStorage(wb, strFilename);
        ...
        public void LoadJPEGFromIsolatedStorage(string strFilename)
        {
            try
            {
                System.IO.Stream s = WP7BarcodeManager.LoadJPEGFromIsolatedStorage(strFilename);
        ...
        private void SetupBarcodeImages(System.IO.Stream ImageStream = null)
        {
            if (BarcodeImage == null || BarcodeImage.PixelWidth == 0)
            {
                if (ImageStream != null)
                {
                    ...
                    if (_angle > 0d)
                    {
                        ImageStream = WP7BarcodeManager.RotateStream(ImageStream, _angle);
        ...
        public WriteableBitmap GetWriteableBitmap(int Width, int Height = 0, System.IO.Stream ImageStream = null)
        {
            ...
            else //called from UI Thread so process image
            {
                try
                {
                    ...
                    Extensions.LoadJpeg(wbBarcodeImage, ImageStream);//Load JPEG from stream into our re-sized writeable bitmap
